...
- **第二階段（`feature/phase-two`，規劃中）**：新增第三張報表「**當月賠款月帳單－T字帳**」——只讀理賠檔，**排除簽單年度 == 設定年**，其餘每個簽單年度各產出一份。目前停在 **M1：等業務方回覆 P-01 ~ P-11**，其中 4 項為阻斷級。**未結案前不要寫 MAPPING / RULE / 程式碼**（見 `..._LOG_第二階段問題追蹤清單.md`）。
...
- **（第二階段）`CLAIM_T_ACCOUNT.xlsx` 跟 `PREMIUM_T_ACCOUNT.xlsx` 只差 A2 標題**（`共保賠款月帳單` vs `共保月帳單`），其餘逐格相同——很容易誤以為可以直接共用同一組 `TAccountCell`，但**兩份範例檔的 Balance Due 落在 G21 而非 G20、還多用了 O20**，這正是 P-03 / P-04 要問業務方的爭議，別自行假設。
- **（第二階段）第二階段的範例檔版面是第一階段「更正前」的舊版**（I3+J3 兩格、O4 含年）。第一階段早就改成 I3 單格 + P4 只寫年，所以那兩份範例的**版面不可照抄**，只有金額 32,460 / 38,331 可信。
