...
- **第二階段·第四張報表「當月共保賠款月帳單－彙總表」（2026-08-17 新增規格）**：規格書 `當月共保賠款月帳單-彙總表規格書v1.0` + 範本 `CLAIM_SUMMARY.xlsx` 已到，**尚未實作**。性質是「第一階段彙整表的逐簽單年度、保費恆為 0 版本」：只讀理賠檔，`B`/`F`/`I` 三欄恆 0、`C` 欄為該簽單年度依公司分群之已決賠款、`G` 欄依成分分攤（中央再保仍為差額法）、合計列 `J23` 恆為 0。已完成 **M7 爭議整理**，列出 **P-14 ~ P-23（🔴 阻斷 3 項）**，**未結案前不進 MAPPING / RULE / 實作**。動工前先讀 `..._LOG_第二階段問題追蹤清單.md` §3.2 / §4 摘要表（二）/ §5.4、現況分析 §11、MAPPING §11。給 PM 的問卷在 `docs/規格來源/第二階段-賠款月帳單/待確認問題6.txt`。
- **待業務方確認的項目**：① **T-29**——以實跑輸出取代 `docs/規格來源/第二階段-賠款月帳單/產出範例/`（同第一階段 T-22）；② **P-14 ~ P-23**（第四張報表）。P-12 / P-13 已於 2026-08-13 獲 PM 回覆結案，B27 / Q-23 隨之修訂。
...
| `..._MAPPING_欄位對照.md` | CSV 欄位索引、中間模型（M1~M8 一階段、M9~M11 二階段）、三張報表的儲存格對照（§7 / §8 / **§10**）、report.json 規格；**§11 為第四張報表之佔位節（待確認，未訂對照）** |
...
| `..._LOG_第二階段問題追蹤清單.md` | **第二階段**：第三張報表 P-01~P-13（**全數結案**）、**§5 定案規則**（含儲存格對照）、M0~M6 里程碑、113/114 驗收基準；**第四張報表 §3.2 事實盤點 + P-14~P-23（⏳ 待確認，🔴 3 項）+ §5.4 待填 + M7~M12** |
...
docs/規格來源/
  ...
  第二階段-賠款月帳單/    〔第三張〕當月賠款月帳單-T字帳規格書v1.0（.md/.docx）、
                          ...
                          CLAIM_SUMMARY.xlsx（範本）、共保保費_當月共保月帳單_彙整表11505_115年.xlsx
                          （隨附範例，❗實為第一階段樣板所產）、共保理賠_當月賠款月帳單.jpg、
                          待確認問題6.txt（給 PM 的 P-14~P-23 問卷）
```
...
- **（第二階段·第四張報表）`CLAIM_SUMMARY.xlsx` 的第 6 列（計算說明列）是舊值**：`D6` = `(4) = (2) + (3)`（應為 `-`）、`I6` = `(9)=(6)x5%`（費率已是 6%）。程式**不覆寫**第 6 列，直接套用會把錯字印在報表上（P-14 待確認）。隨附的「範例檔」也不是本報表產的——A2 標題、工作表名、檔名全是第一階段彙整表的樣貌，**只有金額與儲存格配置可用**（P-20）。
