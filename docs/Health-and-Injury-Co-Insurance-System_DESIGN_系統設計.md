...
| **D-08** | **第四張報表「賠款彙總表」尚未設計**。規格書 v1.0（2026-08-17）已到但 **P-14 ~ P-23 未結案（🔴 3 項）**，依規格先行原則不得先行設計。屆時預期新增 `ClaimSummaryCell` 常數（**不得共用 `SummaryCell`**——`A2` 標題、年度來源、保費恆 0 三處語意不同）與 `ClaimSummaryWriter`，並沿用既有之 `M10`（應產出年度）、成分查表與中央再保差額法。擴充點 X8（一報表多檔）已可涵蓋 | **高**（阻斷實作） |
