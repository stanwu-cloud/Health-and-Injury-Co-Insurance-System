...
> **第二階段開發中**：新增第三張報表「當月賠款月帳單－T字帳」（依簽單年度分別產出多份），於 `feature/phase-two` 分支進行，目前等待業務方確認 11 項爭議，見 `docs/..._LOG_第二階段問題追蹤清單.md`。
...
templet/    Excel 樣板（PREMIUM_T_ACCOUNT / PREMIUM_SUMMARY；CLAIM_T_ACCOUNT 為第二階段備用）
input/      匯入檔（含個資，未納入版控）
output/     產出報表
